...
def initLinkCells(sim, domain):
    gridSize = np.array([int(b/sim.pot.cutoff) for b in domain.localExtent], dtype=np.int)
    boxSize = domain.localExtent*1.0/gridSize
    ...
    return linkcell(nLocalBoxes, nTotalBoxes, gridSize, 1.0/boxSize, domain.localMin, domain.localMax)


class linkcell:
    def __init__(self, nLocalBoxes, nTotalBoxes, gridSize, invBoxSize, localMin, localMax):
        self.MAXATOMS = 64
    ...
    def getBoxFromTuple(self, ix, iy, iz):
        ...
        if iBox < 0:
            print 'iBox less than zero',iBox
            print 'ix,iy,iz',ix,iy,iz
        assert iBox >=0
    ...
    def getBoxFromCoord(self, r):
        ix = int(math.floor( (r[0] - self.localMin[0])*self.invBoxSize[0]))
        iy = int(math.floor( (r[1] - self.localMin[1])*self.invBoxSize[1]))
        iz = int(math.floor( (r[2] - self.localMin[2])*self.invBoxSize[2]))

        if r[0] < self.localMax[0]:
            if ix == self.gridSize[0]:
                ix = self.gridSize[0] - 1
        else:
            ix = self.gridSize[0]

        if r[1] < self.localMax[1]:
            if iy == self.gridSize[1]:
                iy = self.gridSize[1] - 1
        else:
            iy = self.gridSize[1]

        if r[2] < self.localMax[2]:
            if iz == self.gridSize[2]:
